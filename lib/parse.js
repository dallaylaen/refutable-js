...
// Convert Report.toString() back into a same-ish report.

// Hack needed to emulate pending conditions w/o creating a million
// never fulfilling promises
const notGonnaHappen = new Promise( (resolve, reject) => { } );

function parse (...text) {
    // TODO some docs, maybe? Should this be in the index?
    ...
    for (; i < lines.length; i++) {
        ...
        if (found) {
            protocol = found[1]; /* eslint-disable-line no-unused-vars */
            // TODO warn if greated than what we have
            i++;
        }
        break;
    }
    ...
    for (; i < lines.length; i++) {
        ...
        if (s === 'r(') {
            if (stack[0]) {
                const [unusedReport, n, data] = stack[0];
                if (!n)
...
parse.protocol = Report.protocol;
