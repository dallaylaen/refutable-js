'use strict';

function report (...args) {
    const block = args.pop();
    const contract = new Report();
    block(contract, ...args);
    contract.stop();
    return contract;
}

function refute (...args) {
    const contract = report(...args);
    if (!contract.isPassing())
        throw new Error(contract.getTap());
}

class Report {
    constructor() {
        this._count     = 0;
        this._failCount = 0;
        this._descr     = [];
        this._failed    = [];
        this._diag      = [];
        this._nested    = [];
        this._done      = false;
    }

    isDone() {
        return this._done; // is it even needed?
    }

    isPassing() {
        return this._failCount === 0;
    }

    getCount() {
        return this._count;
    }

    getTapLines() {
        // TAP for now, use another format later because "perl is scary"
        const tap = [ '1..'+this._count ];
        // TODO diag[0]
        for( let i = 1; i <= this._count; i++ ) {
            const data = this.getDetails(i);
            if (data.nested) {
                tap.push( '# subcontract:'+(data.name?' '+data.name:'') );
                tap.push( ... data.nested.getTapLines().map( s => '    '+s ));
            }
            tap.push((data.pass?'':'not ') + 'ok ' + i
                + (data.name ? ' - '+data.name : ''));
            tap.push(...data.reason.map(s=>'# '+s));
            tap.push(...data.diag.map(s=>'# '+s));
        }
        if (!this.isPassing())
            tap.push('# Failed');
        return tap;
    }
    getTap() {
        const tap = this.getTapLines();
        tap.push('');
        return tap.join('\n');
    }

    getDetails(n) {
        // TODO validate n

        // ugly but what can I do
        if (n === 0) {
            return {
                test: 0,
                diag: this._diag[0] || [],
            };
        }

        let reason = this._failed[n];
        if (reason && !Array.isArray(reason))
            reason = [reason];

        return {
            test:   n,
            name:   this._descr[n] || '',
            pass:   !reason,
            reason: reason || [],
            diag:   this._diag[n] || [],
            nested: this._nested[n],
        };
    }

    setNextCheck (reason, descr) {
        // TODO die if done
        const n = ++this._count;
        if (descr)
            this._descr[n] = descr;
        if (!reason) {
            // TODO log something
            return 1;
        }

        // TODO maybe if( reason instanceof Report ) but maybe not
        this._failed[n] = reason;
        this._failCount++;
        // TODO explanation et al

        return 0;
    }
    setNestedCheck (report, descr) {
        // TODO die if done, validate report
        const n = ++this._count;
        if (descr)
            this._descr[n] = descr;

        this._nested[n] = report;
        if (report.isPassing())
            return 1;
        this._failed[n] = [];
        this._failCount++;
        return 0;
    }
    diag( ...message ) {
        // TODO preprocess message
        if (!this._diag[this._count])
            this._diag[this._count] = [];
        this._diag[this._count].push( ...message );
    }

    stop() {
        this._done = true;
    }

    getGhost() {
        const sign = ['t'];
        let streak = 0;
        for (let i=1; i <= this._count; i++) {
            if (this._failed[i]) {
                if (streak) sign.push(streak);
                sign.push('N');
                streak = 0;
            } else {
                streak++;
            }
        }
        if (streak) sign.push(streak);
        sign.push('d');
        return sign.join('');
    }
}

refute.addCondition = function(name, options, impl) {
    if (Report.prototype[name])
        throw new Error('name taken: '+name);
    if (typeof options !== 'object')
        throw new Error('bad options');
    if (typeof impl !== 'function')
        throw new Error('bad implementation');

    if (typeof options.args !== 'number')
        throw new Error('args must be a number');

    const minArgs    = options.minArgs || options.args;
    const maxArgs    = options.maxArgs || options.args;
    const descrFirst = options.descrFirst || options.fun;
    const nested     = options.nested;

    // TODO alert unknown options

    let code;

    code = function(...args) {
        const descr = descrFirst
            ? args.shift()
            : (args.length > maxArgs ? args.pop() : undefined);
        if (args.length > maxArgs || args.length < minArgs)
            throw new Error('Bad argument count in condition '+name); // TODO
        if (!nested) {
            const reason = impl( ...args );
            return this.setNextCheck( reason, descr );
        } else {
            const inner = new Report();
            impl( inner, ...args );
            return this.setNestedCheck( inner, descr );
        }
    };

    Report.prototype[name] = code;
};

refute.addCondition(
    'equals',
    {args:2},
    (a,b) => a === b ? 0 : [ 'Got      : '+a, 'Expected : ' + b ]
);
refute.addCondition(
    'matches',
    {args:2},
    (a,rex) => (''+a).match(rex) ? 0 : [
        'String         : '+a,
        'Does not match : '+rex
    ]
);

// TODO addCondition( 'nested', {nested:true}, ... )
Report.prototype.nested = function(name, ...args) {
    // TODO check args
    const child = report(...args);
    this.setNestedCheck( child, name );
};


refute.Report = Report;
refute.report   = report;

module.exports = refute;

