...
class Report {
    ...
    getLines(indent='') {
        const out = ['r('];
        if (this._info[0])
            out.push( ...this._info[0].map( s => '; '+s ) );
        for (let n = 1; n<=this._count; n++) {
            out.push( (this._evidence[n] ? '!':'')+n+'. '+(this._descr[n] || '') );
            if( this._nested[n]) {
                out.push( ...this._nested[n].getLines(indent+'    ') );
            } else if( this._evidence[n] ) {
                out.push( '    ^ Condition '+(this._condName[n] || 'check')
                    +' failed at '+this._where[n] );
                out.push( ...this._evidence[n].map(
                    s => '    '+(s.match(/^[-+^] /)?s:'^ '+s)) );
            };
            if (this._info[n])
                out.push( ...this._info[n].map( s => '; '+s ) );
        };
        out.push(')');
        return out.map( s => indent+s );
    }
