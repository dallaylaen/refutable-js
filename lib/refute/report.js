...
class Report {
    ...
    /**
     *  @desc If mocha is detected, pretend to be a unit test
     *
     */
    getMocha() {
        // TODO throw something better
        if (typeof describe === 'function' && typeof it === 'function') {
            describe( this.getTitle(), () => {
                for( let n = 1; n <= this.getCount(); n++) {
                    const data = this.getDetails(n);
                    it (data.name, done => {
                        if (!data.pass)
                            throw new Error( this.getTapEntry(n) );
                        done();
                    });
                }
            });
        } else {
            console.log(this.getTap());
            if (!this.isPassing())
                throw 'Contract failed: ' + this.getTitle(); // avoid stack trace
        };
    };
